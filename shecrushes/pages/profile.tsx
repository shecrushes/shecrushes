...
const Profile: NextPage = () => {  
  // User status
  const { user, loggedIn, setUser, setUserToken } = useContext(AuthContext)
  ...
  const loadProfileData = useCallback(async() => {
    ...
    if(loading === false && user.displayName) {
      setLoading(true)
      ...
      // Fetch more comments                
      const newComments = await getProfile(user.displayName, page, 5)
  ...
  return (
      <div className="min-h-screen overflow-x-auto font-sans dark bg-trueGray-900">
        <Head>
          <title>SheCrushes - Profile</title>
        </Head>
                    ...
                    <div className='w-full mt-4'>
                      ...
                      {likes &&
                        <div className='grid grid-cols-3 md:grid-cols-4'>
                          {likes.map((video: profileVideoLikes, index: number) => {
                            return (
                              <>
                                <div className="flex items-center" key={`likedVideo:${video.video_id}`}>   
                      ...
                      {likesCount === '0' &&
                        <div className='mt-6 text-center'>
                          <p className='font-medium text-trueGray-300'>Your likes will show up here.</p>
                        </div>
                      }
              ...
              {activeTab === 1 &&
                <>
                  <div className='flex flex-col mt-4'>
                    ...
                    {comments &&
                      <>
                        {comments.map((comment: profileComments, index: number) => {
                          return (<>
                              {/* Comment */}
                              <div key={`comment:${comment.video_id}`}>
                    ...
                    {commentsCount === '0' &&
                      <div className='mt-6 text-center'>
                        <p className='font-medium text-trueGray-300'>Your comments will show up here.</p>
                      </div>
                    }
